...
tracker = CentroidTracker(maxDisappeared=20, maxDistance=20) #(maxDisappeared=80, maxDistance=90)
device='cpu'
# device = 'cuda' if torch.cuda.is_available() else 'cpu'
# model = torch.hub.load('ultralytics/yolov5', 'custom', path="best.pt")#, force_reload=True)
model4 = torch.hub.load('WongKinYiu/yolov7', 'custom', "fire_11may.pt")
# model4.to(device)
...
def plot_boxes(results, frame):
        ...
        for i in range(n):
            ...
            if (row[4] >= 0.70):  # 0.70
                ...
                # # print('...................',c)
                idx = int(labels[i])
                # print('...fire..', classes[int(labels[i])])
                if classes[int(labels[i])] == "Smoke":
                        continue
                if classes[int(labels[i])] == "Stove" and row[4] <= 0.65:
                        continue
                # print('cord',cord)
                co=classes[int(labels[i])]
                bbox_index=i
                # print('co',co)
                (H, W) = frame.shape[:2]
                # print('/',cord[i, 0:4])
